...
public class SpringSecurity {
     ...
     *
     * Аунтефикация в SpringSequrity реалтзована в Session и Cookies
     * Cookes - пара ключ - значение, находятся в памяти браузера, посылается
     * серверу при каждом запросе, у пользователя при успешной аунтефикаци назначается
     * например id=123, сессия - учаток памяти предназначенный для какого-то конкретного пользователя
     * с id=123, там храняться данные пользователя, сессяи на сервере ищется по id позьзователя
     * которое сохранено в Cookies.
     *
     * При каждом запросе для пользователя создается отдельный java-поток, в котрый подгрудается
     * сессия. У куки есть срок действия, поэтому нужно переодически переаунтефицироваться
     *
     *
     * */
}
