...
/**
 *
 * Exchanger<V> (обменник) - используется для обмена данными в
 * определенной точке работы программы, параметризуется
 * типом объекта который нужно передать
 * Точка синхронизации двух потоков, поток, вызывающий
 * метод exchange((V) v) - блокируется и ждет другой поток,
 * когда другой поток вывзает exchange((V) v)
 *
 * */
    ...
    public static class Truck implements Runnable {
        ...
        public void run() {
            try {
                ...
                //остановка потока, ожидание второго, и обмен значениями
